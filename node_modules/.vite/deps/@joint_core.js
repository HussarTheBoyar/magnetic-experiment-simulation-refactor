...
Point.prototype = {
  ...
  move: function(ref, distance) {
    ...
    var offset2 = this.offset(cos2(theta) * distance, -sin2(theta) * distance);
    return offset2;
  },
...
Path.isDataSupported = function(data2) {
...
var V = function() {
  ...
  var cos4 = math.cos;
  var sin4 = math.sin;
  var V2 = function(el, attrs, children2) {
  ...
  VPrototype.text = function(content, opt) {
    ...
    for (var i = 0, lastI = lines.length - 1; i <= lastI; i++) {
      ...
      if (line3) {
        ...
        if (annotations) {
          lineMetrics = {};
          lineAnnotations = V2.findAnnotationsAtIndex(annotations, offset2);
  ...
  VPrototype.children = function() {
    ...
    for (var i = 0; i < children2.length; i++) {
      ...
      if (currentChild.nodeType === 1) {
        outputArray.push(V2(children2[i]));
      }
    }
    return outputArray;
  };
  ...
  VPrototype.sample = function(interval) {
    ...
    while (distance < length2) {
      ...
      samples.push({ x: sample.x, y: sample.y, distance });
      distance += interval;
    }
    return samples;
  };
  ...
  V2.createSvgDocument = function(content) {
    if (content) {
      ...
      const { documentElement: documentElement2 } = V2.parseXML(XMLString, { async: false });
  ...
  V2.createSVGStyle = function(stylesheet) {
    ...
    return node;
  }, V2.createCDATASection = function(data2 = "") {
    const xml = document.implementation.createDocument(null, "xml", null);
    return xml.createCDATASection(data2);
  };
  V2.idCounter = 0;
  ...
  V2.parseXML = function(data2, opt) {
    opt = opt || {};
    var xml;
    try {
      ...
      xml = parser.parseFromString(data2, "text/xml");
    } catch (error) {
      xml = void 0;
    }
  ...
  V2.mergeAttrs = function(a, b) {
    for (var attr2 in b) {
      if (attr2 === "class") {
        ...
        if (V2.isObject(a[attr2]) && V2.isObject(b[attr2])) {
          a[attr2] = V2.mergeAttrs(a[attr2], b[attr2]);
        } else if (V2.isObject(a[attr2])) {
          a[attr2] = V2.mergeAttrs(a[attr2], V2.styleToObject(b[attr2]));
        } else if (V2.isObject(b[attr2])) {
          a[attr2] = V2.mergeAttrs(V2.styleToObject(a[attr2]), b[attr2]);
        } else {
          a[attr2] = V2.mergeAttrs(V2.styleToObject(a[attr2]), V2.styleToObject(b[attr2]));
        }
      } else {
        a[attr2] = b[attr2];
      }
  ...
  V2.shiftAnnotations = function(annotations, index, offset2) {
    if (annotations) {
      annotations.forEach(function(annotation) {
  ...
  V2.rectToPath = function(r) {
    ...
    var topRx = min5(r.rx || r["top-rx"] || 0, width2 / 2);
    var bottomRx = min5(r.rx || r["bottom-rx"] || 0, width2 / 2);
    var topRy = min5(r.ry || r["top-ry"] || 0, height2 / 2);
    var bottomRy = min5(r.ry || r["bottom-ry"] || 0, height2 / 2);
...
var ListCache = class {
  ...
  delete(key) {
    const data2 = this.__data__;
    const index = assocIndexOf(data2, key);
    if (index < 0) {
      return false;
    }
...
var dataPriv = new Data_default();
var dataUser = new Data_default();

// node_modules/@joint/core/src/mvc/Dom/Event.mjs
var Event = function(src, props3) {
  if (!(this instanceof Event)) {
    return new Event(src, props3);
  }
  if (src && src.type) {
...
var document2 = typeof window !== "undefined" ? window.document : null;
var documentElement = document2 && document2.documentElement;
var rTypeNamespace = /^([^.]*)(?:\.(.+)|)/;
...
$.parseHTML = function(string) {
  const context = document2.implementation.createHTMLDocument();
...
var Dom = function(selector) {
  ...
  const el = document2.getElementById(match[2]);
  if (el) {
    this[0] = el;
    this.length = 1;
  }
...
function position() {
  ...
  if ($el.css("position") === "fixed") {
    ...
    const isStaticallyPositioned = (el2) => {
      const { position: position2 } = getComputedStyle(el2);
...
var animationKey = "animationFrameId";
var cssReset = {};
...
var special = /* @__PURE__ */ Object.create(null);
...
Object.assign($.fn, props_default);
...
var parseDOMJSON = function(json, namespace) {
  ...
  const parseNode = function(siblingsDef, parentNode, parentNS) {
    for (let i = 0; i < siblingsDef.length; i++) {
      ...
      const svg2 = ns === svgNamespace;
      const wrapperNode = svg2 ? V_default(node) : Dom_default(node);
...
var flattenObject = function(obj, delim, stop2) {
  delim = delim || "/";
  var ret = {};
  for (var key in obj) {
...
var sanitizeHTML = function(html2) {
  const [outputEl] = Dom_default.parseHTML("<div>" + html2 + "</div>");
...
var setAttributesBySelector = function(element, attrs) {
  var $element = Dom_default(element);
...
assign(Model.prototype, Events, {
  ...
  unset: function(attr2, options) {
    return this.set(attr2, void 0, assign({}, options, { unset: true }));
  },
  ...
  changedAttributes: function(diff2) {
    if (!diff2) return this.hasChanged() ? clone(this.changed) : false;
...
var propsList = Object.keys(props2).map((key) => props2[key]).join("");
var numberPattern = "[-+]?[0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?";
var findSpacesRegex = /\s/g;
var parseFormulaRegExp = new RegExp(`^(${numberPattern}\\*)?([${propsList}])(/${numberPattern})?([-+]{1,2}${numberPattern})?$`, "g");
  ...
  "connection": {
    ...
    set: function({ stubs = 0 }) {
      ...
      if (isFinite(stubs) && stubs !== 0) {
        ...
        const sourceParts = path.divideAtLength(offset2, { segmentSubdivisions });
        const targetParts = path.divideAtLength(-offset2, { segmentSubdivisions });
...
var Cell = Model.extend({
  ...
  constructor: function(attributes2, options) {
    ...
    if (defaults2 = result(this, "defaults")) {
      ...
      attrs = merge({}, defaults2, attrs, customizer);
    }
    this.set(attrs, options);
    this.changed = {};
    this.initialize.apply(this, arguments);
  ...
  toJSON: function(opt) {
    ...
    const finalAttributes = objectDifference(attributes2, omit(defaultAttributes, "id", "type"), { maxDepth: 4 });
  ...
  clone: function(opt) {
    opt = opt || {};
    if (!opt.deep) {
      var clone3 = Model.prototype.clone.apply(this, arguments);
...
var wrapWith = function(object, methods2, wrapper) {
  if (isString(wrapper)) {
    if (!wrappers[wrapper]) {
  ...
  toArray(methods2).forEach(function(method) {
...
function parseFromSVGString(str) {
  ...
  const document3 = parser.parseFromString(markupString, "text/html");
  const svg2 = document3.querySelector("svg");
  return build(svg2);
}
...
var PortData = function(data2) {
  var clonedData = cloneDeep(data2) || {};
  this.ports = [];
  this.groups = {};
...
PortData.prototype = {
  ...
  _getPosition: function(position2, setDefault) {
    var args = {};
    var positionName;
    if (isFunction(position2)) {
      positionName = "fn";
      args.fn = position2;
    } else if (isString(position2)) {
      positionName = position2;
    } else if (position2 === void 0) {
      positionName = setDefault ? "left" : null;
    } else if (Array.isArray(position2)) {
      positionName = "absolute";
      args.x = position2[0];
      args.y = position2[1];
    } else if (isObject(position2)) {
      positionName = position2.name;
      assign(args, position2.args);
    }
    var result2 = { args };
    if (positionName) {
...
var Element2 = Cell.extend({
  ...
  translate: function(tx, ty, opt) {
    ...
    var position2 = this.get("position") || { x: 0, y: 0 };
  ...
  resize: function(width2, height2, opt) {
    ...
    if (opt.direction) {
      ...
      this.set("size", { width: width2, height: height2 }, opt);
  ...
  _fitToElements: function(opt = {}) {
    ...
    let { x, y, width: width2, height: height2 } = elementsBBox;
    const { left: left4, right: right4, top: top4, bottom: bottom4 } = normalizeSides(opt.padding);
    x -= left4;
    y -= top4;
    width2 += left4 + right4;
    height2 += bottom4 + top4;
    let resultBBox = new Rect(x, y, width2, height2);
    if (expandOnly) {
  ...
  getBBox: function(opt = {}) {
    ...
    const { angle = 0, position: { x, y }, size: { width: width2, height: height2 } } = attributes2;
...
var Circle = Element2.define("standard.Circle", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var Ellipse2 = Element2.define("standard.Ellipse", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var Path2 = Element2.define("standard.Path", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var Polygon2 = Element2.define("standard.Polygon", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var Polyline2 = Element2.define("standard.Polyline", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var Image = Element2.define("standard.Image", {
  attrs: {
    root: {
      cursor: "move"
    },
...
var TextBlock = Element2.define("standard.TextBlock", {
  attrs: {
    root: {
      cursor: "move"
    },
...
function buildRoundedSegment(offset2, path, curr, prev, next) {
...
var View = ViewBase.extend({
  ...
  delegateElementEvents: function(element, events, data2) {
    ...
    for (var eventName in events) {
      ...
      if (!method) continue;
      Dom_default(element).on(eventName + eventNS, data2, method.bind(this));
    }
    return this;
  },
...
assign(Collection.prototype, Events, {
  ...
  set: function(models, options) {
    ...
    if (set2.length && replace) {
      orderChanged = this.length !== set2.length || this.models.some(function(m, index) {
        return m !== set2[index];
      });
...
var config2 = [Collection, collectionMethods, "models"];
function addMethods(config5) {
  var Base = config5[0], methods2 = config5[1], attribute = config5[2];
  const methodsToAdd = {
    sortBy,
    toArray
  };
...
var addClass2 = HighlighterView.extend({
  UPDATABLE: false,
  MOUNTABLE: false,
  options: {
    className
...
var list = HighlighterView.extend({
  ...
  highlight(elementView, node) {
    ...
    const { attribute, size = 20, gap = 5, direction = Directions2.ROW } = this.options;
    ...
    const isRowDirection = direction === Directions2.ROW;
    ...
    if (prevItems.length !== items.length || comparison.some((unchanged) => !unchanged)) {
      ...
      const itemsEls = items.map((item, index) => {
        ...
        const offset2 = index * (itemWidth + gap);
        itemEl.setAttribute(
          "transform",
          isRowDirection ? `translate(${offset2}, 0)` : `translate(0, ${offset2})`
        );
        return itemEl;
      });
  ...
  position(element, listSize) {
    const { vel, options } = this;
    const { margin = 5, position: position2 = "top-left" } = options;
    const { width: width2, height: height2 } = element.size();
    const { left: left4, right: right4, top: top4, bottom: bottom4 } = normalizeSides(margin);
    const bbox2 = new Rect(left4, top4, width2 - (left4 + right4), height2 - (top4 + bottom4));
    let { x, y } = getRectPoint(bbox2, position2);
    switch (position2) {
      case Positions.CENTER:
      case Positions.TOP:
      case Positions.BOTTOM: {
...
function alignLine(line3, type, offset2 = 0) {
...
function anchorConnectionPoint(line3, _view, _magnet, opt) {
  let { offset: offset2, alignOffset, align: align2 } = opt;
  if (align2) alignLine(line3, align2, alignOffset);
  return offsetPoint(line3.end, line3.start, offset2);
}
...
function boundaryIntersection(line3, view, magnet, opt) {
  ...
  if (localShape instanceof Path) {
    ...
    if (!data2[BNDR_SUBDIVISIONS]) data2[BNDR_SUBDIVISIONS] = localShape.getSegmentSubdivisions({ precision });
    pathOpt = {
      precision,
...
var normal2 = function(vertices, opt, linkView) {
  return vertices;
};
...
var config3 = {
  ...
  fallbackRouter: function(vertices, opt, linkView) {
    ...
    return orthogonal(vertices, assign({}, config3, opt), linkView);
  },
  /* Deprecated */
...
var manhattan = function(vertices, opt, linkView) {
  return router(vertices, assign({}, config3, opt), linkView);
};
...
var metro = function(vertices, opt, linkView) {
  ...
  return manhattan(vertices, assign({}, config4, opt), linkView);
};
...
var DEFINED_DIRECTIONS = [Directions3.LEFT, Directions3.RIGHT, Directions3.TOP, Directions3.BOTTOM];
var OPPOSITE_DIRECTIONS = {
  [Directions3.LEFT]: Directions3.RIGHT,
  [Directions3.RIGHT]: Directions3.LEFT,
  [Directions3.TOP]: Directions3.BOTTOM,
  [Directions3.BOTTOM]: Directions3.TOP
};
var VERTICAL_DIRECTIONS = [Directions3.TOP, Directions3.BOTTOM];
var ANGLE_DIRECTION_MAP = {
  0: Directions3.RIGHT,
  180: Directions3.LEFT,
  270: Directions3.TOP,
  90: Directions3.BOTTOM
};
...
function resolveSides(source, target) {
  ...
  } else if (sourceDirection === Directions3.ANCHOR_SIDE) {
    sourceSide = sourceBBox.sideNearestToPoint(sourcePoint);
  } else if (sourceDirection === Directions3.MAGNET_SIDE) {
  ...
  } else if (targetDirection === Directions3.ANCHOR_SIDE) {
    targetSide = targetBBox.sideNearestToPoint(targetPoint);
  } else if (targetDirection === Directions3.MAGNET_SIDE) {
...
function resolveForTopSourceSide(source, target, nextInLine) {
  const { x0: sx0, y0: sy0, width: width2, height: height2, point: anchor2, margin } = source;
  ...
  if (tx === ax && ty < sy0) return Directions3.BOTTOM;
  if (tx < ax && ty < smy0) {
    if (nextInLine.point.x === ax) return Directions3.BOTTOM;
    return Directions3.RIGHT;
  }
  if (tx > ax && ty < smy0) {
    if (nextInLine.point.x === ax) return Directions3.BOTTOM;
    return Directions3.LEFT;
  }
  if (tx < smx0 && ty > smy0) return Directions3.TOP;
  if (tx > smx1 && ty > smy0) return Directions3.TOP;
...
function resolveForBottomSourceSide(source, target, nextInLine) {
  ...
  const { x: ax } = anchor2;
  const { x0: tx, y0: ty } = target;
  if (tx === ax && ty > sy1) return Directions3.TOP;
  if (tx < ax && ty > smy1) {
    if (nextInLine.point.x === ax) return Directions3.TOP;
    return Directions3.RIGHT;
  }
  if (tx > ax && ty > smy1) {
    if (nextInLine.point.x === ax) return Directions3.TOP;
  ...
  if (tx < smx0 && ty < smy1) return Directions3.BOTTOM;
  if (tx > smx1 && ty < smy1) return Directions3.BOTTOM;
  ...
  return Directions3.TOP;
}
function resolveForLeftSourceSide(source, target, nextInLine) {
  const { y0: sy0, x0: sx0, width: width2, height: height2, point: anchor2, margin } = source;
  const sx1 = sx0 + width2;
  const sy1 = sy0 + height2;
  const smx0 = sx0 - margin;
  const smy0 = sy0 - margin;
  ...
  const { x0: tx, y0: ty } = target;
  if (tx < ax && ty === ay) return Directions3.RIGHT;
  if (tx <= smx0 && ty < ay) return Directions3.BOTTOM;
  if (tx <= smx0 && ty > ay) return Directions3.TOP;
  if (tx >= smx0 && ty < smy0) return Directions3.LEFT;
  if (tx >= smx0 && ty > smy1) return Directions3.LEFT;
...
function resolveForRightSourceSide(source, target, nextInLine) {
  ...
  const { x: ax, y: ay } = anchor2;
  const { x0: tx, y0: ty } = target;
  if (tx > ax && ty === ay) return Directions3.LEFT;
  if (tx >= smx1 && ty < ay) return Directions3.BOTTOM;
  if (tx >= smx1 && ty > ay) return Directions3.TOP;
  if (tx <= smx1 && ty < smy0) return Directions3.RIGHT;
  if (tx <= smx1 && ty > smy1) return Directions3.RIGHT;
...
function getDirectionForLinkConnection(linkOrigin, connectionPoint, linkView) {
  ...
  if (roundedAngle % 180 === 0 && linkOrigin.y === connectionPoint.y) {
    return linkOrigin.x < connectionPoint.x ? Directions3.LEFT : Directions3.RIGHT;
  } else if (linkOrigin.x === connectionPoint.x) {
    return linkOrigin.y < connectionPoint.y ? Directions3.TOP : Directions3.BOTTOM;
  }
  switch (roundedAngle) {
    case 0:
    case 180:
...
function pointDataFromAnchor(view, point2, bbox2, direction, isPort, fallBackAnchor, margin) {
  if (direction === Directions3.AUTO) {
    direction = isPort ? Directions3.MAGNET_SIDE : Directions3.ANCHOR_SIDE;
  }
...
function getOutsidePoint(side, pointData, margin) {
  ...
  const { x0, y0, width: width2, height: height2 } = pointData;
  switch (side) {
    case "left":
...
function getHorizontalDistance(source, target) {
  ...
  if (source.direction === target.direction) {
    ...
    const boundaryDefiningShape = source.direction === Directions3.TOP ? aboveShape : belowShape;
...
function getVerticalDistance(source, target) {
  ...
  if (source.direction === target.direction) {
    ...
    const boundaryDefiningShape = source.direction === Directions3.LEFT ? leftShape : rightShape;
...
function moveAndExpandBBox(bbox2, direction, margin) {
  switch (direction) {
    case Directions3.LEFT:
      bbox2.inflate(0, margin).moveAndExpand({ x: -margin, width: margin });
      break;
    case Directions3.RIGHT:
      bbox2.inflate(0, margin).moveAndExpand({ width: margin });
      break;
    case Directions3.TOP:
      bbox2.inflate(margin, 0).moveAndExpand({ y: -margin, height: margin });
      break;
    case Directions3.BOTTOM:
      bbox2.inflate(margin, 0).moveAndExpand({ height: margin });
      break;
  }
  return bbox2;
}
...
function getLoopCoordinates(direction, angle, margin) {
  const isHorizontal = direction === Directions3.LEFT || direction === Directions3.RIGHT;
  let dx = 0;
  let dy = 0;
...
function rightAngleRouter(vertices, opt, linkView) {
  const { sourceDirection = Directions3.AUTO, targetDirection = Directions3.AUTO } = opt;
  ...
  if (isVertexInside) {
    ...
    const isVerticalAndAligns = alignsVertically && (resolvedSourceDirection === Directions3.TOP || resolvedSourceDirection === Directions3.BOTTOM);
    const isHorizontalAndAligns = alignsHorizontally && (resolvedSourceDirection === Directions3.LEFT || resolvedSourceDirection === Directions3.RIGHT);
  ...
  if (targetPoint.view && targetPoint.view.model.isElement()) {
    ...
    if (!lastPointOverlap || lastPointOverlap && definedDirection === resolvedTargetDirection) {
      ...
      const isVerticalAndAligns = alignsVertically && (resolvedTargetDirection === Directions3.TOP || resolvedTargetDirection === Directions3.BOTTOM);
      const isHorizontalAndAligns = alignsHorizontally && (resolvedTargetDirection === Directions3.LEFT || resolvedTargetDirection === Directions3.RIGHT);
...
function resolveDirection(from, to) {
  ...
  if (isDirectionVertical) {
    ...
    if (accessDirection === Directions3.BOTTOM) {
      sourceDirection = isToAbove ? OPPOSITE_DIRECTIONS[accessDirection] : dx >= 0 ? Directions3.RIGHT : Directions3.LEFT;
      if (dx > 0) {
        targetDirection = isToAbove ? Directions3.LEFT : Directions3.TOP;
      } else if (dx < 0) {
        targetDirection = isToAbove ? Directions3.RIGHT : Directions3.TOP;
      }
    } else {
      sourceDirection = isToAbove ? dx >= 0 ? Directions3.RIGHT : Directions3.LEFT : OPPOSITE_DIRECTIONS[accessDirection];
      if (dx > 0) {
        targetDirection = isToAbove ? Directions3.BOTTOM : Directions3.LEFT;
      } else if (dx < 0) {
        targetDirection = isToAbove ? Directions3.BOTTOM : Directions3.RIGHT;
      }
    }
  } else {
    const isToLeft = from.point.x > to.point.x;
    const dy = to.point.y - from.point.y;
    if (accessDirection === Directions3.RIGHT) {
      sourceDirection = isToLeft ? OPPOSITE_DIRECTIONS[accessDirection] : dy >= 0 ? Directions3.BOTTOM : Directions3.TOP;
      if (dy > 0) {
        targetDirection = isToLeft ? Directions3.TOP : Directions3.LEFT;
      } else if (dy < 0) {
        targetDirection = isToLeft ? Directions3.BOTTOM : Directions3.LEFT;
      }
    } else {
      sourceDirection = isToLeft ? dy >= 0 ? Directions3.BOTTOM : Directions3.TOP : OPPOSITE_DIRECTIONS[accessDirection];
      if (dy > 0) {
        targetDirection = isToLeft ? Directions3.RIGHT : Directions3.TOP;
      } else if (dy < 0) {
        targetDirection = isToLeft ? Directions3.RIGHT : Directions3.BOTTOM;
      }
    }
  }
  return [sourceDirection, targetDirection];
}
rightAngleRouter.Directions = Directions3;
...
var Graph = Model.extend({
  ...
  transferCellEmbeds: function(sourceCell, targetCell, opt = {}) {
    ...
    targetCell.embed(children2, { ...opt, reparent: true });
    this.stopBatch(batchName);
  },
  ...
  _filterAtPoint: function(cells, point2, opt = {}) {
    return cells.filter((el) => el.getBBox({ rotate: true }).containsPoint(point2, opt));
  },
  ...
  _getFindUnderElementGeometry: function(element, searchBy = "bbox") {
    const bbox2 = element.getBBox({ rotate: true });
  ...
  startBatch: function(name, data2) {
    ...
    return this.trigger("batch:start", assign({}, data2, { batchName: name }));
  },
  stopBatch: function(name, data2) {
    data2 = data2 || {};
    this._batches[name] = (this._batches[name] || 0) - 1;
    return this.trigger("batch:stop", assign({}, data2, { batchName: name }));
  },
...
var calcAttributes = calcAttributesList.reduce((acc, attrName) => {
  acc[attrName] = true;
  return acc;
}, {});
var positiveValueAttributes = positiveValueList.reduce((acc, attrName) => {
  acc[attrName] = true;
  return acc;
}, {});
...
var CellView = View.extend({
  ...
  getRootTranslateMatrix: function() {
    ...
    var mt = V_default.createSVGMatrix().translate(position2.x, position2.y);
    return mt;
  },
...
var ElementView = CellView.extend({
  ...
  getRotateString: function() {
    ...
    const { width: width2, height: height2 } = this.model.size();
  ...
  processEmbedding: function(data2 = {}, evt, x, y) {
    ...
    if (newCandidateView && newCandidateView != prevCandidateView) {
      ...
      data2.candidateEmbedView = newCandidateView.highlight(
  ...
  _disallowUnembed: function(data2) {
    const { model, whenNotAllowed = "revert" } = data2;
    ...
    switch (whenNotAllowed) {
      ...
      case "revert": {
        const { initialParentId, initialPosition, initialZIndices } = data2;
        if (initialPosition) {
  ...
  pointermove: function(evt, x, y) {
    ...
    const { targetMagnet, action, delegatedView } = data2;
    if (targetMagnet) {
  ...
  drag: function(evt, x, y) {
    ...
    var element = this.model;
    var data2 = this.eventData(evt);
    var { pointerOffset, restrictedArea, embedding } = data2;
...
var LinkView = CellView.extend({
  ...
  presentationAttributes: {
    ...
    labels: [Flags3.LABELS, Flags3.TOOLS],
    labelMarkup: [Flags3.LABELS],
    vertices: [Flags3.UPDATE],
  ...
  confirmUpdate: function(flags, opt) {
    ...
    if (this.hasFlag(flags, Flags3.RENDER)) {
      ...
      this.updateHighlighters(true);
      this.updateTools(opt);
      flags = this.removeFlag(flags, [Flags3.RENDER, Flags3.UPDATE, Flags3.LABELS, Flags3.TOOLS, Flags3.CONNECTOR]);
  ...
  renderStringMarkup: function(markup) {
    var children2 = V_default(markup);
  ...
  _getLabelStringMarkup: function(labelMarkup2) {
    var children2 = V_default(labelMarkup2);
  ...
  dragArrowheadStart: function(evt, x, y) {
    ...
    var data2 = this.startArrowheadMove(arrowheadType, { ignoreBackwardsCompatibility: true });
    this.eventData(evt, data2);
  },
  dragStart: function(evt, x, y) {
    if (this.isDefaultInteractionPrevented(evt)) return;
  ...
  dragLabel: function(evt, x, y) {
    var data2 = this.eventData(evt);
    var label = { position: this.getLabelPosition(x + data2.dx, y + data2.dy, data2.positionAngle, data2.positionArgs) };
    if (this.paper.options.snapLabels) delete label.position.offset;
    const setOptions2 = { ui: true };
  ...
  drag: function(evt, x, y) {
    var data2 = this.eventData(evt);
    this.model.translate(x - data2.dx, y - data2.dy, { ui: true });
    this.eventData(evt, {
      dx: x,
      dy: y
    });
  ...
  _disallow: function(data2) {
    switch (data2.whenNotAllowed) {
      ...
      case "revert":
      default:
        this.model.set(data2.arrowhead, data2.initialEnd, { ui: true });
        break;
    }
  },
  ...
  _snapArrowhead: function(evt, x, y) {
    ...
    var prevClosestView = data2.closestView || null;
    var prevClosestMagnet = data2.closestMagnet || null;
    var prevMagnetProxy = data2.magnetProxy || null;
      ...
      candidates.forEach((candidate) => {
        ...
        if (distance < minDistance) {
          ...
          if (isAlreadyValidated || paper.options.validateConnection.apply(
            paper,
            data2.validateConnectionArgs(view, view.el === magnet ? null : magnet)
          )) {
            minDistance = distance;
    ...
    if (closestView) {
      const { prevEnd, prevX, prevY } = data2;
      data2.prevX = x;
      data2.prevY = y;
      isSnapped = true;
      if (!newClosestMagnet) {
  ...
  _snapArrowheadEnd: function(data2) {
    ...
    if (closestView && closestMagnet) {
      closestView.unhighlight(data2.magnetProxy, { connecting: true, snapping: true });
  ...
  _connectArrowhead: function(target, x, y, data2) {
    ...
    if (data2.eventTarget !== target) {
      ...
      if (viewUnderPointer) {
        ...
        if (magnetUnderPointer && this.paper.options.validateConnection.apply(
          paper,
          data2.validateConnectionArgs(viewUnderPointer, magnetUnderPointer)
        )) {
          if (magnetProxy) {
    ...
    model.set(data2.arrowhead, { x, y }, { ui: true });
  },
  _connectArrowheadEnd: function(data2 = {}, x, y) {
    const { model } = this;
    const { viewUnderPointer, magnetUnderPointer, magnetProxy, arrowhead } = data2;
  ...
  _afterArrowheadMove: function(data2) {
    if (data2.z !== null) {
      this.model.set("z", data2.z, { ui: true });
      data2.z = null;
    }
...
var GridLayer = PaperLayer.extend({
  style: {
    "pointer-events": "none"
  },
  _gridCache: null,
  ...
  renderGrid() {
    ...
    gridSettings.forEach((gridLayerSetting, index) => {
      ...
      let vPattern;
      if (!refs.exist(id)) {
        vPattern = V_default("pattern", { id, patternUnits: "userSpaceOnUse" }, V_default(options.markup));
  ...
  updateGrid() {
    ...
    const { x, y, width: width2, height: height2 } = paper.getArea();
    vSvg.attr({ x, y, width: width2, height: height2 });
  ...
  _getGridRefs() {
    ...
    const svgVEl = V_default("svg", { width: "100%", height: "100%" }, [defsVEl]);
    grid = this._gridCache = {
      root: svgVEl,
      patterns: {},
      add: function(id, patternVEl) {
        const rectVEl = V_default("rect", { width: "100%", height: "100%", fill: `url(#${id})` });
  ...
  */
  ...
  onCellAdded: function(cell, _, opt) {
    ...
    if (this.isAsync() || !isNumber(position2)) {
      ...
      if (opt.maxPosition === position2) this.freeze({ key: "addCells" });
      this.renderView(cell, opt);
      if (position2 === 0) this.unfreeze({ key: "addCells" });
    }
  },
  ...
  render: function() {
    ...
    const { svg: svg2, defs, layers } = childNodes;
  ...
  addStylesheet: function(css2) {
    if (!css2) return;
    V_default(this.svg).prepend(V_default.createSVGStyle(css2));
  },
  createLayer(name) {
    switch (name) {
  ...
  scale: function(sx, sy, data2) {
    ...
    if (sx === void 0) {
      return V_default.matrixToScale(ctm);
    }
    if (sy === void 0) {
      sy = sx;
    }
  ...
  scaleUniformAtPoint: function(scale2, point2, data2) {
    ...
    const matrix = V_default.createSVGMatrix().translate(
      tx - point2.x * (scale2 - sx),
      ty - point2.y * (scale2 - sy)
    ).scale(scale2, scale2);
  ...
  translate: function(tx, ty, data2) {
    const ctm = this.matrix();
    if (tx === void 0) {
      return V_default.matrixToTranslate(ctm);
    }
    tx || (tx = 0);
    ty || (ty = 0);
  ...
  matrix: function(ctm, data2 = {}) {
    ...
    if (!currentTransformString && V_default.matrixToTransformString() === ctmString) {
      return this;
    }
    ...
    if (e !== prev.e || f !== prev.f) {
      this.trigger("translate", e, f, data2);
    }
  ...
  updateViewsAsync: function(opt, data2) {
    opt || (opt = {});
    data2 || (data2 = { processed: 0, priority: MIN_PRIORITY });
    const { _updates: updates, options } = this;
    const id = updates.id;
    if (id) {
      cancelFrame(id);
  ...
  fitToContent: function(gridWidth, gridHeight, padding, opt) {
    ...
    const { x, y, width: width2, height: height2 } = this.getFitToContentArea(opt);
  ...
  pointerdown: function(evt) {
    ...
    if (isContextMenu) {
      ...
      const contextmenuEvt = new Dom_default.Event(evt.originalEvent, { type: "contextmenu", data: evt.data });
  ...
  pointerup: function(evt) {
    ...
    if (!normalizedEvt.isPropagationStopped()) {
      this.pointerclick(new Dom_default.Event(evt.originalEvent, { type: "click", data: evt.data }));
    }
    this.delegateEvents();
  },
  mouseover: function(evt) {
  ...
  delegateDragEvents: function(view, data2) {
    data2 || (data2 = {});
    this.eventData({ data: data2 }, { sourceView: view || null, mousemoved: 0 });
  ...
  defineGradient: function(gradient) {
    ...
    const stopVEls = toArray(stops).map(({ offset: offset2, color, opacity: opacity2 }) => {
  ...
  gridPatterns: {
    ...
    fixedDot: [{
      ...
      update: function(el, opt, paper) {
        ...
        V_default(el).attr({ width: width2, height: height2 });
      }
    }],
    mesh: [{
      color: "#AAAAAA",
...
var Segments = ToolView.extend({
  ...
  snapHandle: function(handle, position2, data2) {
    ...
    var prev = vertices[index - 2] || data2.sourceAnchor;
    var next = vertices[index + 1] || data2.targetAnchor;
    var snapRadius = this.options.snapRadius;
...
var Arrowhead = ToolView.extend({
  ...
  update: function() {
    ...
    var matrix = V_default.createSVGMatrix().translate(position2.x, position2.y).rotate(angle);
    const { scale: scale2 } = this.options;
    if (scale2) matrix = matrix.scale(scale2);
...
var RotateLabel = Control.extend({
  xAxisVector: new Point(1, 0),
  children() {
    const {
      buttonColor = "#333",
  ...
  getPosition(view) {
    const { offset: offset2 = 0 } = this.options;
    const { x = 0, y = 0 } = typeof offset2 === "number" ? { x: 0, y: offset2 } : offset2;
...
var Button = ToolView.extend({
  ...
  getElementMatrix() {
    ...
    let { x = 0, y = 0, offset: offset2 = {}, useModelGeometry, rotate, scale: scale2 } = options;
    let bbox2 = getViewBBox(view, useModelGeometry);
    const angle = view.model.angle();
    if (!rotate) bbox2 = bbox2.bbox(angle);
    const { x: offsetX = 0, y: offsetY = 0 } = offset2;
    if (isPercentage(x)) {
      x = parseFloat(x) / 100 * bbox2.width;
    } else if (isCalcExpression(x)) {
      x = Number(evalCalcExpression(x, bbox2));
  ...
  getLinkMatrix() {
    const { relatedView: view, options } = this;
    const { offset: offset2 = 0, distance: distanceOpt = 0, rotate, scale: scale2 } = options;
    ...
    let matrix = V_default.createSVGMatrix().translate(position2.x, position2.y).rotate(angle).translate(0, offset2);
    if (!rotate) matrix = matrix.rotate(-angle);
    if (scale2) matrix = matrix.scale(scale2);
    return matrix;
  },
...
var HoverConnect = Connect.extend({
  ...
  getButtonMatrix() {
    ...
    const { offset: offset2 = 0, distance = 0, rotate, scale: scale2 } = options;
    let tangent, position2, angle;
    if (isPercentage(distance)) {
  ...
  canShowButton() {
    return Dom_default.event.has(this.paper.el);
  },
  showButton() {
...
var layout = { PortLabel: portLabel_exports, Port: port_exports };
...
@joint/core/src/mvc/Dom/Dom.mjs:
  (*!
