...
export class UMagnetElement extends SpeedTrackingElement {
        private fieldVisuals: SVGSVGElement[] = [];
        private fieldOffsets: { verticalOffset: number }[] = [];
        private readonly imageSize = { width: 120, height: 160 };
        private poleS: { x: number; y: number } = { x: 0, y: 0 };
        private poleN: { x: number; y: number } = { x: 0, y: 0 };
        ...
        private toggleControlIcons(paper: joint.dia.Paper) {
                ...
                toggleIcon.style.top = `${paperRect.top + pos.y - 10}px`;
        ...
        public showField(container: HTMLElement) {
                ...
                this.poleS = { x: pos.x + 10, y: yCenter }

                this.poleN = { x: pos.x + this.imageSize.width - 10, y: yCenter }

                const numberOfClosedArcs = 10;
                const arcSpacing = 10;
